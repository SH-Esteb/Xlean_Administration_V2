hook.Add("HUDPaint", "Xlean_Admin_OverHead", function()
    if xlean_admin_config.Staff[LocalPlayer():GetUserGroup()] then
        local status = LocalPlayer():GetNWInt("XAdmin::Admin")
        ...
        if status == 1 then
            for k, v in pairs(player.GetAll()) do
                ...
                if v:Alive() and v ~= LocalPlayer() then
                    if (v:IsAdmin()) then
                        draw.DrawText("★", "XLean_Admin:15", x - 2, y - 20, Color(200, 255, 255, 200), 1, 0, TEXT_ALIGN_CENTER) -- https://seafightpkp.jimdofree.com/caract%C3%A8res-sp%C3%A9ciaux/
                    else
                        draw.DrawText("●", "XLean_Admin:15", x - 2, y - 20, Color(200, 255, 255, 200), 1, 0, TEXT_ALIGN_CENTER) -- https://seafightpkp.jimdofree.com/caract%C3%A8res-sp%C3%A9ciaux/
                    end
                end
                ...
                if v:GetPos():Distance(LocalPlayer():GetPos()) < 2500 and v:Alive() and v ~= LocalPlayer() then
                    ...
                    if heath < 100 then
                        ...
                        draw.RoundedBox(0, x - 50, y - 53, 100, 5, Color(200, 0, 0, 255))
                    end
                    if armor < 100 then
                        draw.RoundedBox(0, x - 50, y - 45, armor, 5, Color(0, 0, 200, 255))
                    else
                        draw.RoundedBox(0, x - 50, y - 45, 100, 5, Color(0, 0, 200, 255))
                    end
                    draw.DrawText(v:GetName(), "XLean_Admin:15", x, y - 80, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER)
                end
            end
        end
    end
